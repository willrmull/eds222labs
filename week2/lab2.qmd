...
df <- read_csv(here::here('week2', 'data', 'la_storm.csv'))

# Create a plot of the gage height and precipitation data
ggplot(df, aes(x = precip_wk_in, y = gage_ht)) +
  geom_point()
...
**I used weekly precipitation as the x axis and gage height as the y axis. This implies that gage height is explained by weekly precipitation.**
...
model <- lm(gage_ht ~ precip_wk_in, data = df)
...
$\beta_0$ : What the gage height is when there is no rain
...
y_bar <- mean(df$gage_ht)
x <- df$precip_wk_in
y_hat <- 3.9 + 2.2 * x
SST <- sum((df$gage_ht - y_bar)^2)
SSE <- sum((df$gage_ht - y_hat)^2)
#$What fraction of gage height have we explained
R2 <- (SST-SSE) / SST
...
y is the observed values of y

y bar is the mean value of the observed heights

y hat is the estimated values of the heights
...
rreas_murres$rreas_anchovy_index
rreas_murres$comu_rockfish_diet
...
ggplot(rreas_murres) +
  geom_point(aes(x = comu_anchovy_diet, y = rreas_anchovy_index, color = "red")) +
  geom_point(aes(x = comu_rockfish_diet, y = rreas_rockfish_index, color = "blue"))
